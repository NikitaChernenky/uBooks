...
routes.post('/create', (req, res) => { // Create a new customer in the database. Using Stored Procedure.
    ...
    mySqlConnection.query('INSERT INTO Users (`UserID`, `Name`, `Surname`, `Email`, `Password`, `AdminRole`) VALUES (?, ?, ?, ?, ?, 0); INSERT INTO Customers (`UserID`, `CountryID`, `PhoneNumber`, `CardNumber`) VALUES (?, ?, ?, 0);', [customer.UserID, customer.Name, customer.Surname, customer.Email, customer.Password, customer.UserID, customer.CountryID, customer.PhoneNumber, customer.CardNumber], (error, results, fields) => {
        if (!error) {
