...
routes.post('/create', (req, res) => { // Create a new customer in the database. Using Stored Procedure.
    ...
    mySqlConnection.query('CALL CreateCustomer (?, ?, ?, ?, ?, ?, ?)', [customer.Name, customer.Surname, customer.Email, customer.Password, customer.CountryID, customer.PhoneNumber, customer.CardNumber], (error, results, fields) => {
        if (!error) {
