...
export class CustomerMyBooksComponent implements OnInit {
  ...
  createBook() {
    ...
    if (
      ...
      if (
        !this.books.find(
          book =>
            (book.ISBN == this.data.ISBN && book.BookID != this.data.BookID) ||
            (book.BookAuthor == this.data.BookAuthor &&
              book.BookTitle == this.data.BookTitle &&
              book.BookTitle == this.data.BookTitle &&
              book.PublicationYear == this.data.PublicationYear &&
        ...
        let book = {
          ...
          BookGenreID: this.bookgenres.find(
            genre => genre.BookGenreID == this.data.BookGenreID
          ).BookGenreID,
          BookGenre: this.bookgenres.find(
            genre => genre.BookGenreID == this.data.BookGenreID
          ).BookGenre,
        ...
        this.booksService.insertBook(book).subscribe(() => {
          this.fetchData();
        });
        /* Hide adding form. */
        this.visibleForm = "";
      } else {
        alert("The same book exists!");
        this.visibleForm = "";
      }
    } else {
  ...
  updateBook() {
    /* Checking if data properties is not empty. */
    if (
      this.changingData.ISBN &&
      this.changingData.BookCover &&
      this.changingData.BookTitle &&
      this.changingData.BookAuthor &&
      this.changingData.BookGenreID != "none" &&
      this.changingData.PublicationYear &&
      this.changingData.BookOverview &&
      this.changingData.Quantity >= 0 &&
      this.changingData.Price
    ) {
      ...
      if (
        !this.books.find(
          book =>
            (book.ISBN == this.changingData.ISBN &&
              book.BookID != this.changingData.BookID) ||
        ...
        this.booksService.updateBook(this.changingData).subscribe(() => {
          this.fetchData();
        });
        /* Update book's data. */
        this.books.find(book => {
          if (book.BookID == this.changingData.BookID) {
            ...
            book.BookTitle = this.changingData.BookTitle;
            book.BookAuthor = this.changingData.BookAuthor;
            book.BookGenreID = this.bookgenres.find(
              genre => genre.BookGenreID == this.changingData.BookGenreID
            ).BookGenreID;
            book.BookGenre = this.bookgenres.find(
              genre => genre.BookGenreID == this.changingData.BookGenreID
            ).BookGenre;
            book.PublicationYear = this.changingData.PublicationYear;
  ...
  getBookForUpdate(id) {
    //[was called updateBook function before]
  ...
  cardNumberAvailability() {
    if (
      ...
      this.currentUser.CardNumber != null
    ) {
  ...
  onSelectFile(event, action) {
    ...
    if (action == "update") {
      ...
      if (event.target.files[0]) {
        this.changingData.BookCover =
          "assets/img/" + event.target.files[0].name;
      } else {
        this.changingData.BookCover = "";
      }
    }
