...
export class CustomerCartComponent implements OnInit {
        ...
        for (let i = 0; i < book.length; i++) {
          for (let j = 0; j < this.data.length; j++) {
            if (book[i].BookID == this.data[j]) {
              ...
              this.alltotalprice +=
                book[i].Price *
                this.books[this.books.length - 1].BookQuantityToBuy;
            }
          }
        }
      },
  ...
  deleteBookFromCart(id) {
    ...
    for (let i = 0; i < this.books.length; i++) {
      if (this.books[i].BookID == id) {
        this.alltotalprice -=
          this.books[i].Price * this.books[i].BookQuantityToBuy;
  ...
  /* Increment book's quantity. */
  plusQuantity(id) {
    if (
      this.books.find(book => book.BookID == id).BookQuantityToBuy <
      this.books.find(book => book.BookID == id).Quantity
    ) {
      this.books.find(book => book.BookID == id).BookQuantityToBuy++;
